...
bool ViewingModel::CheckFittingVertices(GLint *viewport, GLdouble *modelview,
	...
	REP(mesh,mMesh.size()) {
		...
		double dist;
		for (unsigned int loopVer = 0; loopVer < mMesh[mesh].vertex.size();
				loopVer += 3) {
...
void ViewingModel::Load3DSModel(void) {
	...
	for (int loop = 0; loop < m_model->nmeshes; ++loop) {
		...
		for (int loopX = 0; loopX < mesh->nfaces; ++loopX) {
			//1頂点目
			memcpy(&mMesh[loop].normal[loopX * 9], &normal[loopX][0],
					sizeof(float) * 3);
			memcpy(&mMesh[loop].vertex[loopX * 9],
					&mesh->vertices[mesh->faces[loopX].index[0]][0],
					sizeof(float) * 3);
			//２頂点目
			memcpy(&mMesh[loop].normal[loopX * 9 + 3], &normal[loopX][0],
					sizeof(float) * 3);
			memcpy(&mMesh[loop].vertex[loopX * 9 + 3],
					&mesh->vertices[mesh->faces[loopX].index[1]][0],
					sizeof(float) * 3);
			//3頂点目
			memcpy(&mMesh[loop].normal[loopX * 9 + 6], &normal[loopX][0],
					sizeof(float) * 3);
			memcpy(&mMesh[loop].vertex[loopX * 9 + 6],
			...
			for (int idmesh = 0; idmesh < 3; idmesh++) {
				...
				//
				mMesh[loop].ind[loopX * 3 + idmesh] +=
						mesh->faces[loopX].index[idmesh];
	...
	for(int texNumber=0; texNumber<sModel->nmeshes; texNumber++) {
		...
		for (unsigned int loopVer = 0; loopVer < nVertices; loopVer++) {
					...
					|| isTriangles > 0)
			{
				...
				sModel->meshes[texNumber]->faces[nFaces].index[isTriangles] =
						loopVer;

				sModel->meshes[texNumber]->vertices[loopVer][0] =
						tmpMesh->mVertices[indices[loopVer]].point.x;
				sModel->meshes[texNumber]->vertices[loopVer][1] =
						tmpMesh->mVertices[indices[loopVer]].point.y;
				sModel->meshes[texNumber]->vertices[loopVer][2] =
						tmpMesh->mVertices[indices[loopVer]].point.z;

				sModel->meshes[texNumber]->texcos[loopVer][0] =
						(tmpMesh->mTextureCoords[indices[loopVer]].x
								- tmpMesh->mTexMin.x) * ratio_x / (W_WIDTH / 2);
				sModel->meshes[texNumber]->texcos[loopVer][1] = ((W_HEIGHT / 2
						- (tmpMesh->mTextureCoords[indices[loopVer]].y
...
bool ViewingModel::LoadMatrix(void) {
	...
	REP(meshloop,MeshNum) {
		for (int verloop = 0; verloop < (int) mMesh[meshloop].nIndex; verloop +=
				3) {
...
void ViewingModel::RenewMeshDataConstruct(const int & separateNumber) {
	...
	REP(loop,num) {
		...
		for (int loopv = 0; loopv < nIndices; loopv++) {
			double decomValue =
					static_cast<double>(mHarmonicValue[mMesh[loop].ind[loopv]]);
...
void ViewingModel::QueryNormal(const int & outer_loop, const int & mesh_index,
		GLdouble * normal) {
	REP(id,3) {
		normal[id] = static_cast<GLdouble>(mMesh[outer_loop].normal[mesh_index
				* 3 + id]);
	}
}

void ViewingModel::QueryVertex(const int & outer_loop, const int & mesh_index,
		GLdouble * vertex) {
	REP(id,3) {
		vertex[id] = static_cast<GLdouble>(mMesh[outer_loop].vertex[mesh_index
				* 3 + id]);
	}
}

void ViewingModel::QueryAmbient(const int & outer_loop, const int & mesh_index,
		GLfloat * ambient) {
...
double ViewingModel::QueryVertexColor(const int & outer_loop,
		const int & mesh_index) const {
...
ViewingModel::ViewingModel(const char * name) :
		mSumOfVertices(0), mSumOfIndices(0), mScales(5.0), mIsConvert(false), mIsLoadMatrix(
				false), mHasTexture(false), mMeshSelected(false), mModelname(
				name) {
	//  mModelname = name;
	REP(i,3) {
		mTrans[i] = 0.0;
